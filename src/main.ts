...
import type { Dir, GameState, RulesOptions } from './rules.js';
import * as content from './content.js';
import * as session from './session.js';
import type { Settings, ActiveSession } from './session.js';
...
let settings: Settings = session.loadSettings();
...
async function api(path: string, init?: RequestInit): Promise<unknown | null> {
  try {
    ...
    if (!res.ok) {
      if (res.status === 429) announce('Server is busy; try again shortly.');
      return null;
    }
    setOnline(true);
    return body;
...
async function submitDaily(sess: ActiveSession): Promise<void> {
  ...
  }) as { ok?: boolean; rank?: number; error?: string } | null;
...
function setHudVisible(v: boolean): void {
  ...
  if (!v) { $('hud-timer-wrap').hidden = true; }
}
...
function themeNow(): content.ThemeDef {
  return ui.themeById(settings.highContrast ? 'mono' : settings.theme);
}

function startRun(mode: ActiveSession['mode'], contentId: string, opts: RulesOptions, ranked: boolean): void {
  ...
  updateAll();
  audio.ensureAudio();
  if (sess.ranked && sess.mode === 'daily') void loadLeaderboard();
}

function setupBoard(st: GameState): void {
  const t = themeNow();
...
function updateAll(): void {
  ...
  let empties = 0;
  for (const row of st.board) for (const v of row) if (v === 0) empties++;
...
function finishRun(): void {
  ...
  if (!sess || runFinished || !sess.state.over) return;
  runFinished = true;
  const st = sess.state;
  const total = rules.scoreTotal(st);
  // records — capture the previous best before it is overwritten below
  const prevBest = progress.bestScores[sess.contentId] ?? 0;
  if (prevBest < total) progress.bestScores[sess.contentId] = total;
  progress.gamesPlayed++;
  if (sess.mode === 'daily') {
    const day = session.utcToday();
    if (!progress.dailyDays.includes(day)) progress.dailyDays.push(day);
  }
  if (sess.mode === 'journey' && st.won) {
    const stage = content.JOURNEY.find((s) => s.id === sess.contentId);
    if (stage && !progress.journeyCompleted.includes(stage.index)) progress.journeyCompleted.push(stage.index);
  }
  if (sess.mode === 'learn' && st.won) progress.tutorialDone = true;
  stopCountdown();
  ...
  if (st.won) audio.sfx.milestone(); else audio.sfx.gameOver();
  if (sess.mode === 'daily') void submitDaily(sess);
}
...
function doMove(dir: Dir): void {
  const sess = session.getActive();
  if (!sess || paused || currentScreen !== 'play') return;
  ...
  if (pendingCommands.has(base)) return;
  ...
  for (let i = 0; i < sess.state.size; i++)
    for (let j = 0; j < sess.state.size; j++) {
      if (prevBoard[i][j] === 0 && sess.state.board[i][j] !== 0) before.add(`${i},${j}`);
    }
  ...
  if (lastMove) {
    ...
    for (let i = 0; i < sess.state.size; i++)
      for (let j = 0; j < sess.state.size; j++)
        if (sess.state.board[i][j] > prevBoard[i][j] && !before.has(`${i},${j}`)) merged.add(`${i},${j}`);
  }
  ...
  hideInvalid();
  checkLessonProgress();
  if (r.gameOver) scheduleFinish(sess, 450);
}

/** Let the final move land on screen before the results take over. */
function scheduleFinish(sess: ActiveSession, delayMs: number): void {
  finishScheduled = true;
  setTimeout(() => {
    if (session.getActive() !== sess) return;
    finishScheduled = false;
    if (currentScreen === 'play') finishRun();
  }, delayMs);
}
...
function hideInvalid(): void { $('invalid-box').hidden = true; }

let countdownIv: ReturnType<typeof setInterval> | null = null;

function stopCountdown(): void {
  if (countdownIv !== null) { clearInterval(countdownIv); countdownIv = null; }
...
function onKey(e: KeyboardEvent): void {
  ...
  if (currentScreen === 'play' && !paused) {
    ...
    if (e.key === 'h' || e.key === 'H') { e.preventDefault(); doHint(); return; }
  }
  if (e.key === 'Escape' || e.key === 'p' || e.key === 'P') {
    if (currentScreen === 'play') { e.preventDefault(); togglePause(); }
    else if (!$('overlay-pause').hidden) { e.preventDefault(); togglePause(false); }
  }
}

/* touch/pointer swipe with capture + tap/drag thresholds */
let ptrStart: { x: number; y: number; t: number; id: number } | null = null;
...
function pollGamepad(): void {
  ...
  if (gp && currentScreen === 'play' && !paused) {
    ...
    for (const [on, d] of dirs) {
      ...
      if (on && !padPrev[key]) doMove(d);
      padPrev[key] = on;
    }
    if ((pressed(9) || pressed(1)) && !padPrev[9]) togglePause();
...
function bindLifecycle(): void {
  document.addEventListener('visibilitychange', () => {
    if (document.hidden && currentScreen === 'play' && !paused) togglePause(true);
  });
...
function openModeSetup(mode: string): void {
...
function startModeFromSetup(): void {
  const mode = $('mode-setup').dataset.mode ?? 'practice';
  if (mode === 'practice') {
    const goal = +($('practice-goal') as HTMLSelectElement).value;
    const size = +($('practice-size') as HTMLSelectElement).value;
    const seed = (Date.now() ^ (Math.random() * 0xffffffff)) >>> 0;
    startRun('practice', `practice-${goal}-${size}`, { seed, size, goalTile: goal, allowUndo: true }, false);
  } else if (mode === 'challenge') {
    const kind = ($('challenge-kind') as HTMLSelectElement).value;
    const table: Record<string, { opts: RulesOptions; id: string }> = {
...
function buildJourneyGrid(): void {
  ...
  for (const s of content.JOURNEY) {
    ...
    if (s.moveLimit) bits.push(`${s.moveLimit} moves max`);
...
let activeLesson: content.LessonDef | null = null;
function startLesson(l: content.LessonDef): void {
  activeLesson = l;
...
function checkLessonProgress(): void {
  ...
  if (l.id === 'lesson-merge') doneMoves = doneMoves && st.log.some((m) => m.merges.length > 0);
  if (l.id === 'lesson-survive') doneMoves = st.maxTile >= 32;
...
function loop(time: number): void {
  ...
  if (document.hidden) return; // zero rendering heartbeat while hidden
  pollGamepad();
  const sess = session.getActive();
  if (currentScreen === 'play' && sess && !paused) {
    session.advanceClock();
    if (sess.state.timeLimitMs > 0) updateAll();
  ...
  if (glActive && currentScreen === 'play') render.frame(time);
}
...
function bindButtons(): void {
  ...
  $('btn-resume').addEventListener('click', () => {
    ...
    if (sess) { runFinished = false; finishScheduled = false; paused = false; setupBoard(sess.state); show('play'); fitCanvas(); updateAll(); }
  });
  for (const card of document.querySelectorAll<HTMLElement>('#mode-cards .card')) {
