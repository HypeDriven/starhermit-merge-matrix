 ...
 */
import { createGame, applyMove, legalMoves, scoreTotal, DIRS, type RulesOptions, type GameState } from './rules.js';
...
export interface StageDef {
  ...
  parMoves: number;       // mastery target
  theme: string;
  tutorialFlag: string | null;
  mastery: boolean;       // periodic mastery stage
}
...
export interface LessonDef {
  ...
  seed: number;
  goalTile: number;
  requireMoves: number;   // player must perform this many legal moves
  hintDir?: string;
}
...
export interface ThemeDef {
  ...
  grid: string;
  cell: string;
  tiles: Record<number, string>; // value → color; keys below 0 = fallback
  textLight: boolean;
  accent: string;
}
...
export function journeyStages(): StageDef[] {
...
export const JOURNEY: StageDef[] = journeyStages();

export function stageOptions(s: StageDef): RulesOptions {
  return { seed: s.seed, size: s.size, goalTile: s.goalTile, moveLimit: s.moveLimit, timeLimitMs: s.timeLimitMs, allowUndo: !s.mastery };
}

export function lessonOptions(l: LessonDef): RulesOptions {
  return { seed: l.seed, size: 4, goalTile: l.goalTile, allowUndo: true };
}

/** Offline validator: legality, reachable goal, bounded duration, no soft lock at start. */
export interface ValidationIssue { stage: string; issue: string }

export function validateStage(s: StageDef, maxSimMoves = 4000): ValidationIssue[] {
  const issues: ValidationIssue[] = [];
  if (!s.id || s.seed < 0 || s.goalTile < 8) issues.push({ stage: s.id, issue: 'bad-fields' });
  if (s.size < 2 || s.size > 8) issues.push({ stage: s.id, issue: 'bad-size' });
  // basic legality: initial state must have at least one legal move
  const st = createGame(stageOptions(s));
  if (legalMoves(st).length === 0) issues.push({ stage: s.id, issue: 'initial-softlock' });
  ...
  for (let m = 0; m < maxSimMoves && !sim2.over; m++) {
    if (sim2.maxTile >= sim2.goalTile) { reached = true; break; }
    let applied = false;
    const order: typeof DIRS = ['down', 'left', 'right', 'up'];
  ...
  if (sim2.maxTile >= sim2.goalTile) reached = true;
  if (!reached && !sim2.over) issues.push({ stage: s.id, issue: 'unbounded-duration' });
...
export function validateAll(): ValidationIssue[] {
  ...
  for (const s of JOURNEY) out.push(...validateStage(s));
  return out;
}

/** Difficulty score from depth/constraint parameters, not just number size. */
export function difficultyOf(s: StageDef): number {
  let d = Math.log2(s.goalTile) * 10;
  if (s.moveLimit > 0) d += 30;
  if (s.timeLimitMs > 0) d += 25;
  if (s.size > 4) d += 10;
  if (!s.mastery === false) d += 5;
  if (s.mastery) d += 10;
  return Math.round(d);
}
