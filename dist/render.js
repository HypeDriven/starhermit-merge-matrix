...
let cellMeshes = [];
let theme = null;
let cfg = { quality: 'high', reducedMotion: false };
let size = 4;
let canvasEl = null;
let disposed = false;
let shake = 0;
...
export function initRender(canvas, settings) {
    ...
    const ground = new THREE.Mesh(new THREE.PlaneGeometry(30, 30), new THREE.MeshStandardMaterial({ color: 0x05070f, roughness: 0.95, metalness: 0.1 }));
...
export function buildBoard(newSize) {
    ...
    // grid base slab
    const slab = new THREE.Mesh(new THREE.BoxGeometry(size + 0.24, 0.1, size + 0.24), new THREE.MeshStandardMaterial({ color: new THREE.Color(theme.grid), roughness: 0.4, metalness: 0.5 }));
    slab.position.y = -0.2;
    boardGroup.add(slab);
    ...
    for (let r = 0; r < size; r++) {
        for (let c = 0; c < size; c++) {
            ...
            if (!t || t.value !== v) {
                ...
                tiles[r][c] = t;
                if (changed?.has(key) && !cfg.reducedMotion)
                    t.mesh.scale.setScalar(0.2); // spawn pop-in
            }
            if (merged?.has(key) && !cfg.reducedMotion)
                t.pulse = 1;
        }
    }
}
...
export function updateQuality(q) {
    cfg = q;
    if (renderer)
        renderer.setPixelRatio(q.quality === 'low' ? 1 : q.quality === 'medium' ? 1.5 : Math.min(devicePixelRatio, 2));
}
